      ...
      <div className='flex justify-between items-center max-w-3xl'>
        ...
        <div className='flex gap-4'>
          <button onClick={() => { setFilter('Approved') }} className={`shadow-custom-sm border rounded-full px-4 py-1 
            cursor-pointer text-xs text-primary ${filter === 'Approved' ? 'text-primary' :
          ...
          <button onClick={() => { setFilter('Not Approved') }} className={`shadow-custom-sm border rounded-full px-4 py-1 
            cursor-pointer text-xs text-primary ${filter === 'Not Approved' ? 'text-primary' :
              'text-gray-700'}`}>Not Approved</button>
        </div>
      </div>
      <div className='relative h-4/5 max-w-3xl overflow-x-auto mt-4 bg-white shadow rounded-lg scrollbar-hide'>
        <table w-full text-sm text-gray-500>
          <thead text-xs text-gray-700 text-left uppercase>
            <tr>
